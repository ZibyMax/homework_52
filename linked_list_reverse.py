...
ОТВЕТ:
Complexity: O(n)  # Для перестройки списка длиной n в обратном порядке потребуется n операций
Memory complexity: O(1)  # Дополнительная пемать не потребуется
...
# Решение: функция перестройки списка в обратном порядке
    def reverse(self):
        if self.head:
            current = self.head
            new_next = None
